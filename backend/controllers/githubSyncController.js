...
function extractChangedFiles(commits) {
  const files      = new Map();
  const slugPattern = /(?:^|\/)0*\d+[-_][a-z0-9-_]+\.[a-z]+$/i;
  for (const commit of commits) {
    for (const f of [...(commit.added || []), ...(commit.modified || [])]) {
      if (slugPattern.test(f) && !f.endsWith('.md') && !f.includes('README')) {
...
exports.handleWebhook = async (req, res) => {
  ...
  for (const { path, sha } of changedFiles) {
    const parsed = parseSlug(path.split('/').pop());
    if (!parsed) {
...
exports.manualSync = async (req, res) => {
  ...
  try {
    ...
    const codeFiles = tree.filter(f =>
      slugPattern.test(f.path) && !f.path.endsWith('.md') && !f.path.includes('README')
    );

    console.log(`[WEBHOOK] Full sync: ${codeFiles.length} files in ${owner}/${repo}`);

    for (const file of codeFiles) {
      const parsed = parseSlug(file.path.split('/').pop());
